...
Two arrays of type uint8_t are allocated and some values are written to the first one.
Then, the VHDL simulation is executed, where the (external) array/buffer is used.
When the simulation is finished, the results are checked. The content of the buffer is
printed both before and after the simulation.
